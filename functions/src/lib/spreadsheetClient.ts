...
export class SpreadsheetClient {
  ...
  async getValues(sheetId: string, range = "A2:C"): Promise<SearchItem[]> {
  ...
  async setValues(sheetId: string, values: SearchItem) {
    ...
    return await this.client.spreadsheets.values.append({
      spreadsheetId: sheetId,
      range: `A${targetRow}:C${targetRow}`,
      valueInputOption: "RAW",
      requestBody: {
        values: [[values.word, values.description, values.link]],
      },
    });
  }
